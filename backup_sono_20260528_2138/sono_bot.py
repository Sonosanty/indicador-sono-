...
class SonoBot:
    def __init__(self):
        ...
        self.config = {
            'BTC': {'min_trade': 10, 'risk_per_trade': 0.50, 'symbol': 'BTC_USDC', 'quote': 'USDC', 'decimals': 6},
            'ETH': {'min_trade': 10, 'risk_per_trade': 0.50, 'symbol': 'ETH_USDC', 'quote': 'USDC', 'decimals': 4},
            'SOL': {'min_trade': 10, 'risk_per_trade': 0.50, 'symbol': 'SOL_USDT', 'quote': 'USDT', 'decimals': 2},
            'XRP': {'min_trade': 10, 'risk_per_trade': 0.50, 'symbol': 'XRP_USDT', 'quote': 'USDT', 'decimals': 1},
            'ALT': {'min_trade': 10, 'risk_per_trade': 0.40, 'symbol': 'ALT_USDT', 'quote': 'USDT', 'decimals': 4},
            'OSMO': {'min_trade': 10, 'risk_per_trade': 0.40, 'symbol': 'OSMO_USDT', 'quote': 'USDT', 'decimals': 4},
        }
    ...
    def run_once(self):
        ...
        try:
            ...
            for asset in ASSETS:
                ...
                # ═══ SWING: solo 2 posiciones máximas ═══
                active_positions = sum(1 for a in ASSETS if self.find_position(a))
                
                # Si ya hay 2 posiciones, no abrir más (swing concentrado)
                if active_positions >= 2 and not pos:
                    logger.info(f'{asset}: Score {score["total"]} - saltado (ya hay {active_positions} posiciones abiertas, máx 2)')
                    continue

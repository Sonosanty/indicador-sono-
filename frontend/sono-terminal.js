...
function updateUI(sc){$('smScore').textContent=sc.total;$('heroScore').textContent=sc.total;$('heroScoreNum').textContent=sc.total;$('heroBar').style.width=sc.total+'%';var se=$('smSignal');if(se){se.textContent=sc.signal;se.className='score-signal '+(sc.signal.indexOf('COMPRA')>=0?'score-buy':sc.signal.indexOf('VENTA')>=0||sc.signal==='CAPITULACION'?'score-sell':sc.signal==='ACUMULAR'?'score-accum':'score-neutral')}$('heroSignal').textContent=sc.signal;$('heroDecision').textContent=sc.decision;$('heroZone').textContent=sc.zone;$('smDecision').textContent=sc.decision+' - '+sc.zone;$('smP1').textContent='P1: '+sc.p1+'/35';$('smP2').textContent='P2: '+sc.p2+'/35';$('smP3').textContent='P3: '+sc.p3+'/30';$('smRSI').textContent='RSI: '+fn(sc.rsi,1);$('heroRsi').textContent=fn(sc.rsi,1);$('smADX').textContent='ADX: '+fn(sc.adx,1);$('heroAdx').textContent=fn(sc.adx,1);$('heroTime').textContent=ts()}
...
function ap(){if(!PD)return;var u=PD.usd,c=PD.chg,eu=u*ER,up=c>=0;$('btcUsdLive').textContent='$'+fn(u,2);$('btcEurLive').textContent=String.fromCharCode(8364)+fn(eu,2);$('previewPrice').textContent='$'+fn(u,2);$('previewChange').textContent=(up?'+':'')+fn(c,2)+'%';var bdg=$('btcChange24hLive');if(bdg){bdg.textContent=(up?'+':'')+fn(c,2)+'% 24h';bdg.className='badge '+(c>.3?'positivo':c<-.3?'negativo':'neutro')}$('lastUpdateBTC').textContent='Actualizado: '+ts()}
...
async function loadCG(){try{var d=await fj(CG+'/global');var g=d.data,mp=g.market_cap_percentage;$('btcDominanceValue').textContent=(mp.btc||0).toFixed(2)+'%';$('ethDominanceValue').textContent=(mp.eth||0).toFixed(2)+'%';$('altsDominanceValue').textContent=(100-(mp.btc||0)-(mp.eth||0)).toFixed(2)+'%';$('marketCapValue').textContent=fb(g.total_market_cap?.usd||0);$('volumeValue').textContent='Vol 24h: '+fb(g.total_volume?.usd||0);var fg=parseInt($('fearValue')?.textContent)||50;var bd=mp.btc||50;var state='',text='',score=50;if(fg<25&&bd>55){state='ACUMULACION';text='Miedo extremo + dominancia alta';score=28}else if(fg<40){state='PRECAUCION';text='Miedo en el mercado';score=45}else if(fg<55){state='NEUTRAL';text='Sin direccion clara';score=55}else if(fg<75){state='OPTIMISMO';text='Confianza creciente';score=72}else{state='EUFORIA';text='Greed extremo, posible techo';score=85}$('macroState').textContent=state;$('macroText').textContent=text;$('macroScore').textContent=score;var ec=$('cardMacroState');if(ec)ec.className='estado '+(score<35?'macro-bear':score<55?'macro-neutral':'macro-bull');$('lastUpdateMacro').textContent='Actualizado: '+ts();$('cryptoRegimeLabel').textContent=state;$('cryptoRegimeDetail').textContent=text+' | Dom '+bd.toFixed(1)+'% | F&G '+fg}catch(ex){}}
...
document.querySelectorAll('.asset-btn').forEach(function(btn){btn.addEventListener('click',function(){document.querySelectorAll('.asset-btn').forEach(function(b){b.classList.remove('active')});this.classList.add('active');var a=this.dataset.asset;var sm={BTC:'BTCUSDT',ETH:'ETHUSDT',SOL:'SOLUSDT',XRP:'XRPUSDT'};var s=sm[a]||'BTCUSDT';$('assetLabel').textContent=a;$('previewAsset').textContent=a+'USDT - LIVE';lp(s);updateScore(s)})});
...
ap = function(){if(!PD)return;var u=PD.usd,c=PD.chg,up=c>=0;
...
loadFN = async function(){try{var d=await fj("https://api.alternative.me/fng/?limit=1&format=json");var v=parseInt(d.data[0].value),lbl=d.data[0].value_classification;$("fear-greed").textContent=v;$("fg-change").textContent=lbl}catch(ex){}};

loadCG = async function(){try{var d=await fj(CG+"/global");var g=d.data,mp=g.market_cap_percentage;$("btc-dom").textContent=(mp.btc||0).toFixed(1)+"%";$("dom-change").textContent="ETH "+(mp.eth||0).toFixed(1)+"%";}catch(ex){}};

loadVIX = async function(){try{var d=await fj(VM);VIX=d;var v=parseFloat(d.vix)||0;$("vix-value").textContent=v.toFixed(2);$("vix-change").textContent=(d.change||0).toFixed(1)+"%"}catch(ex){if(!VIX){$("vix-value").textContent="--";$("vix-change").textContent=""}}};

loadRSI3D = async function(){try{var d=await fj(B+"/klines?symbol=BTCUSDT&interval=3d&limit=20");var cl=d.map(function(k){return parseFloat(k[4])});var r=rsi(cl,14);var v=r[r.length-1];var sig=$("signals-tbody");if(sig){var txt=(v<30?"SOBREVENTA":v>70?"SOBRECOMPRA":"NEUTRAL");sig.innerHTML="<tr><td>"+txt+"\u002Ftd><td>RSI 3D: "+fn(v,1)+"\u002Ftd><td>"+(v<30?"COMPRA":v>70?"VENTA":"ESPERAR")+"\u002Ftd><td>-"+"\u002Ftd>\u002Ftr>";}}catch(ex){}};

// Asset selector - v3 uses .coin-tab
document.querySelectorAll(".coin-tab").forEach(function(btn){btn.addEventListener("click",function(){document.querySelectorAll(".coin-tab").forEach(function(b){b.classList.remove("active")});this.classList.add("active");var a=this.dataset.coin;var sm={BTC:"BTCUSDT",ETH:"ETHUSDT",SOL:"SOLUSDT",XRP:"XRPUSDT"};var s=sm[a]||"BTCUSDT";currentSymbol=s;updateScore();lp();})});

// Live update timers
setInterval(function(){loadAllData()},10000);
...
function updateExecutiveDashboard(){
  ...
  // Regime: 5 niveles
  var regime,cls;
  if(score>=75){regime="BULLISH";cls="risk-on";}
  else if(score>=60){regime="RISK ON";cls="risk-on";}
  else if(score>=45){regime="CAUTIOUS";cls="transition";}
  else if(score>=30){regime="RISK OFF";cls="risk-off";}
  else{regime="CRISIS";cls="risk-off";}
  ...
  if(fg<25){liqText="EXPANSIVA";liqColor="green";liqTrend="+2.8% semanal";}
  else if(fg<40){liqText="POSITIVA";liqColor="green";liqTrend="+1.5% semanal";}
  else if(fg<60){liqText="NEUTRAL";liqColor="yellow";liqTrend="estable";}
  else if(fg<75){liqText="CONTRACCION";liqColor="red";liqTrend="-1.3% semanal";}
  else{liqText="NEGATIVA";liqColor="red";liqTrend="-2.1% semanal";}
  ...
  if(score>=70){riskText="ELEVADO";riskColor="red";}
  else if(score>=55){riskText="MODERADO";riskColor="yellow";}
  else if(score>=40){riskText="BAJO";riskColor="blue";}
  else{riskText="MINIMO";riskColor="green";}
  ...
  if(fg<15){sentText="PANICO";sentColor="green";sentTrend="Extremo historico";}
  else if(fg<30){sentText="MIEDO";sentColor="green";sentTrend="Zona de acumulacion";}
  else if(fg<40){sentText="PREOCUPACION";sentColor="yellow";sentTrend="Mejorando";}
  else if(fg<55){sentText="NEUTRAL";sentColor="yellow";sentTrend="Sin direccion";}
  else if(fg<70){sentText="OPTIMISMO";sentColor="blue";sentTrend="Confianza creciente";}
  else if(fg<85){sentText="GREED";sentColor="red";sentTrend="Cuidado sobrecompra";}
  else{sentText="EUFORIA";sentColor="red";sentTrend="Riesgo de correccion";}
...
function updateHeatmap(){
  ...
  var items=[
    {id:"hm-btc",sym:"BTC",chg:(btcChg>0?"+":"")+btcChg+"%",cls:btcChg>0?"bullish":"bearish",arrow:btcChg>0?"▲":"▼"},
    {id:"hm-eth",sym:"ETH",chg:(ethChg>0?"+":"")+ethChg+"%",cls:ethChg>0?"bullish":"bearish",arrow:ethChg>0?"▲":"▼"},
    {id:"hm-sp500",sym:"S&P500",chg:(sp500Chg>0?"+":"")+sp500Chg+"%",cls:sp500Chg>0?"bullish":"bearish",arrow:sp500Chg>0?"▲":"▼"},
    {id:"hm-nasdaq",sym:"NASDAQ",chg:(nasdaqChg>0?"+":"")+nasdaqChg+"%",cls:nasdaqChg>0?"bullish":"bearish",arrow:nasdaqChg>0?"▲":"▼"},
    {id:"hm-dxy",sym:"DXY",chg:(dxyChg>0?"+":"")+dxyChg+"%",cls:dxyChg>0?"bearish":"bullish",arrow:dxyChg>0?"▼":"▲"},
    {id:"hm-sol",sym:"SOL",chg:(solChg>0?"+":"")+solChg+"%",cls:solChg>0?"bullish":"bearish",arrow:solChg>0?"▲":"▼"},
    {id:"hm-xrp",sym:"XRP",chg:(xrpChg>0?"+":"")+xrpChg+"%",cls:xrpChg>0?"bullish":"bearish",arrow:xrpChg>0?"▲":"▼"},
    {id:"hm-gold",sym:"GOLD",chg:(goldChg>0?"+":"")+goldChg+"%",cls:goldChg>0?"bullish":"bearish",arrow:goldChg>0?"▲":"▼"},
    {id:"hm-bonds",sym:"BONDS",chg:(bondsChg>0?"+":"")+bondsChg+"%",cls:bondsChg>0?"bearish":"bullish",arrow:bondsChg>0?"▼":"▲"},
  ];
...
function loadAllData(){try{$("connection-status").textContent="Conectando...";Promise.all([lp(),updateScore(),loadFN(),loadCG(),loadVIX(),loadRSI3D()]).then(function(){$("connection-status").textContent="Conectado";updateExecutiveDashboard();updateHeatmap();updateAIPanel();}).catch(function(){$("connection-status").textContent="Error de conexión";})}catch(e){}}

setInterval(function(){updateExecutiveDashboard();updateHeatmap();updateAIPanel();},10000);

document.addEventListener("DOMContentLoaded",function(){document.querySelectorAll(".coin-tab").forEach(function(b){b.classList.remove("active");if(b.dataset.coin==="BTC")b.classList.add("active")});loadAllData()});

// ═══════════════════════════════════════════════════════════════
// Motor matemático — indicadores técnicos puros
// ═══════════════════════════════════════════════════════════════
...
// ── SCORE MAESTRO con escala 0-6 (estilo "NEUTRAL · 2/6") ──────
export const computeScore = (candles) => {
  ...
  // Etiqueta estilo "NEUTRAL · 2/6"
  let label, level, action, biasColor
  if      (total >= 78) { label='COMPRA FUERTE'; level=6; action='LONG';            biasColor='success' }
  else if (total >= 62) { label='COMPRA';        level=5; action='LONG PRUDENTE';   biasColor='success' }
  else if (total >= 52) { label='ACUMULACIÓN';   level=4; action='ACUMULAR';        biasColor='primary' }
  else if (total >= 42) { label='NEUTRAL';       level=3; action='ESPERAR';         biasColor='primary' }
  else if (total >= 30) { label='DISTRIBUCIÓN';  level=2; action='REDUCIR';         biasColor='warning' }
  else if (total >= 18) { label='VENTA';         level=1; action='SHORT';           biasColor='danger'  }
  else                  { label='CAPITULACIÓN';  level=0; action='CASH/FUERA';      biasColor='danger'  }

  return {
    total, level,
    label, action, biasColor,
    p1, p2, p3, p1d, p2d, p3d,

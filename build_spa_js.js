...
let existing = '';
try { existing = fs.readFileSync('spa-controller.js', 'utf8'); } catch(e) {}

// Clean up stub content if any (remove lines after "RANGOS")
const idx = existing.indexOf('// RANGOS');
if (idx > 0) existing = existing.substring(0, idx);

fs.writeFileSync('spa-controller.js', existing + code, 'utf8');
console.log('OK: ' + fs.statSync('spa-controller.js').size + ' bytes');

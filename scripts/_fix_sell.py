...
# Para MARKET SELL, Pionex espera 'size' (cantidad base del activo), NO 'amount'
# Para MARKET BUY con amountType=QUOTE, espera 'amount' (cantidad en quote)

print("=== SELL SOL usando 'size' en vez de 'amount' ===")
r=post_order({'symbol':'SOL_USDT','side':'SELL','type':'MARKET','size':'0.769'})
print(json.dumps(r,indent=2)[:400])
time.sleep(2)

print("\n=== SELL ETH usando 'size' ===")
r2=post_order({'symbol':'ETH_USDT','side':'SELL','type':'MARKET','size':'0.0191'})
...
h={'PIONEX-KEY':k,'PIONEX-SIGNATURE':sg,'Content-Type':'application/json'}
r3=requests.get(base+'/api/v1/account/balances?'+q,headers=h,timeout=15).json()
if r3.get('result'):
    for b in r3['data']['balances']:
